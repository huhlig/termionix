...
use crate::{Blink, Color, ColorMode, Intensity, SegmentedString, Underline};
use std::ops::Range;
...
impl StyledString {
    ...
    /// }));
    /// ```
    pub fn from_string(str: &str, style: Option<Style>) -> StyledString {
        StyledString {
            segments: vec![Segment {
                range: 0..str.len(),
    ...
    ///     ..Default::default()
    /// }));
    /// let len = styled.styled_len(&ColorMode::Basic);
    /// assert!(len > 5); // Longer than just "Hello" due to ANSI codes
    /// ```
    pub fn styled_len(&self, mode: &ColorMode) -> usize {
        let mut total = 0;

        for segment in &self.segments {
            // Count the style codes
            let mut temp = String::new();
            segment.style.write_style(mode, &mut temp).unwrap_or(());
            total += temp.len();
    ...
    pub fn concat(&mut self, str: &str) {
        Self::concat_with_style(self, str, Style::default());
    }
    ...
    /// });
    /// ```
    pub fn concat_with_style(&mut self, str: &str, style: Style) {
        self.segments.push(Segment {
    ...
    ///
    /// ```
    /// use termionix_ansicodes::{StyledString, Style, Color, Underline};
    ///
    ...
    /// ```
    pub fn set_style(&mut self, style: Style, range: Range<usize>) {
    ...
    /// - [`Style`] - The style type used in SGR segments
    ...
    pub fn write_str<W: std::fmt::Write>(
        ...
        for segment in &self.segments {
            ...
            segment.style.write_style(mode, writer)?;
...
fn parse_ansi_codes(codes: &[&str], style: &mut Style) {
    let mut i = 0;
    while i < codes.len() {
        match codes[i].parse::<u8>() {
            Ok(0) => *style = Style::default(), // Reset
...
mod tests {
    use super::*;
    use crate::{Blink, Intensity, Underline};
    ...
    fn test_from_string_no_style() {
        ...
        assert_eq!(styled.segments[0].style, Style::default());
    }
    ...
    fn test_parse_multiple_segments() {
        ...
        assert_eq!(styled.segments[1].style, Style::default());

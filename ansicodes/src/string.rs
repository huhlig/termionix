...
use crate::{AnsiConfig, Blink, Color, Intensity, SegmentedString, Underline};
use std::ops::Range;
...
impl StyledString {
    ...
    /// ```
    pub fn styled_len(&self, config: Option<&AnsiConfig>) -> usize {
        let mut total = 0;

        for segment in &self.segments {
            // Count the style codes
            let mut temp = String::new();
            segment.style.write_style(&mut temp, config).unwrap_or(());
            total += temp.len();
    ...
    pub fn write_str<W: std::fmt::Write>(
        ...
        for segment in &self.segments {
            ...
            segment.style.write_style(writer, config)?;
...
mod tests {
    use super::*;
    use crate::{Blink, ColorMode, Intensity, Underline};

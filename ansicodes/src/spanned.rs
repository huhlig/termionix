...
use crate::{CSICommand, ControlCode, EraseInDisplayMode, EraseInLineMode, SegmentedString};
        ...
        while pos < bytes.len() {
            ...
            match bytes[pos] {
                ...
                0x00..=0x1A | 0x1C..=0x1F | 0x7F => {
                    if let Some(control_code) = ControlCode::from_byte(bytes[pos]) {
                        pos += 1;
                        // Greedy: consume consecutive identical control codes
                        while pos < bytes.len() {
                            if let Some(next_code) = ControlCode::from_byte(bytes[pos]) {
                ...
                0x80..=0x9F => {
                    ...
                    if let Some(control_code) = ControlCode::from_byte(bytes[pos]) {
                        ...
                        while pos < bytes.len() && bytes[pos] >= 0x80 {
                            // Check if it's a C1 control code
                            if bytes[pos] <= 0x9F && ControlCode::from_byte(bytes[pos]).is_some() {
                                break;
                            }
                            let char_len = utf8_char_len(bytes[pos]);
        ...
        for span in &self.0 {
            match span {
                ...
                Span::Control { value, .. } => {
                    ...
                    segmented.push_control(*value);
                }
                Span::Escape { .. } => {
                    // Push standalone escape character
                    segmented.push_segment(crate::segment::Segment::Escape);
                }
                Span::CSI { value, .. } => {
                    // Push CSI command
                    segmented.push_segment(crate::segment::Segment::CSI(value.clone()));
                }
                Span::OSC { range } => {
                    // Extract OSC data
                    if let Some(data) = source.get(range.clone()) {
                        segmented
                            .push_segment(crate::segment::Segment::OSC(data.as_bytes().to_vec()));
                    }
                }
                Span::DCS { range } => {
                    // Extract DCS data
                    if let Some(data) = source.get(range.clone()) {
                        segmented
                            .push_segment(crate::segment::Segment::DCS(data.as_bytes().to_vec()));
                    }
                }
                Span::SOS { range } => {
                    // Extract SOS data
                    if let Some(data) = source.get(range.clone()) {
                        segmented
                            .push_segment(crate::segment::Segment::SOS(data.as_bytes().to_vec()));
                    }
                }
                Span::ST { range } => {
                    // Extract ST data
                    if let Some(data) = source.get(range.clone()) {
                        segmented
                            .push_segment(crate::segment::Segment::ST(data.as_bytes().to_vec()));
                    }
                }
                Span::PM { range } => {
                    // Extract PM data
                    if let Some(data) = source.get(range.clone()) {
                        segmented
                            .push_segment(crate::segment::Segment::PM(data.as_bytes().to_vec()));
                    }
                }
                Span::APC { range } => {
                    // Extract APC data
                    if let Some(data) = source.get(range.clone()) {
                        segmented
                            .push_segment(crate::segment::Segment::APC(data.as_bytes().to_vec()));
                    }
                }
            }
        }
...
fn parse_csi_command(param_bytes: &[u8], final_byte: Option<u8>) -> CSICommand {
    ...
    match final_byte {
        // Cursor movement
        b'A' => CSICommand::CursorUp(params.get(0).copied().unwrap_or(1)),
        b'B' => CSICommand::CursorDown(params.get(0).copied().unwrap_or(1)),
        b'C' => CSICommand::CursorForward(params.get(0).copied().unwrap_or(1)),
        b'D' => CSICommand::CursorBack(params.get(0).copied().unwrap_or(1)),
        b'E' => CSICommand::CursorNextLine(params.get(0).copied().unwrap_or(1)),
        b'F' => CSICommand::CursorPreviousLine(params.get(0).copied().unwrap_or(1)),
        b'G' => CSICommand::CursorHorizontalAbsolute(params.get(0).copied().unwrap_or(1)),
        b'H' | b'f' => CSICommand::CursorPosition {
        ...
        b'n' if params.get(0) == Some(&6) => CSICommand::DeviceStatusReport,
...
mod tests {
    ...
    fn test_all_c0_controls() {
        ...
        assert_eq!(ControlCode::from_byte(0x00), Some(ControlCode::NUL));
        assert_eq!(ControlCode::from_byte(0x09), Some(ControlCode::HT));
        assert_eq!(ControlCode::from_byte(0x0A), Some(ControlCode::LF));
        assert_eq!(ControlCode::from_byte(0x0D), Some(ControlCode::CR));
        assert_eq!(ControlCode::from_byte(0x7F), Some(ControlCode::DEL));
    }

...
///
/// ```rust
/// use termionix_ansicodes::{Style, Color, ColorMode, Intensity, AnsiConfig};
/// use std::fmt::Write;
///
...
impl Style {
    ...
    pub fn len(&self, _config: Option<&AnsiConfig>) -> usize {
        let mut length = 0;
    ...
    ///
    /// ```rust
    /// use termionix_ansicodes::{AnsiConfig, ColorMode, Style};
    /// use std::fmt::Write;
    ...
    ///
    /// ```
    pub fn write_reset<W: std::fmt::Write>(
        writer: &mut W,
        config: Option<&AnsiConfig>,
    ) -> std::fmt::Result {
        if let Some(config) = config && config.color_mode.is_ansi(){
            write!(writer, "\x1b[0m")
        } else {
            Ok(())
        }
    ...
    ///
    /// ```rust
    /// use termionix_ansicodes::{Style, Color, ColorMode, Intensity, Underline, Blink, AnsiConfig};
    /// use std::fmt::Write;
    ///
    /// let config = AnsiConfig::enabled();
    /// let style = Style {
    ...
    /// };
    ///
    /// let ansi_code = style.to_string(Some(&config));
    ...
    ///
    /// ```rust
    /// use termionix_ansicodes::{Style, ColorMode, AnsiConfig};
    ///
    /// let config = AnsiConfig::enabled();
    /// let style = Style::default();
    /// let ansi_code = style.to_string(Some(&config));
    ...
    ///
    /// ```rust
    /// use termionix_ansicodes::{Style, Color, ColorMode, AnsiConfig};
    ///
    /// let config_none = AnsiConfig::strip_all();
    /// let config_fixed = AnsiConfig::fixed_color_only();
    /// let config_true = AnsiConfig::true_color_only();
    /// let style = Style {
    ///     foreground: Some(Color::RGB(255, 100, 50)),
    ///     ..Default::default()
    /// };
    ...
    /// assert_eq!(style.to_string(Some(&config_none)), "");
    ///
    /// // Basic colors convert RGB to nearest basic color
    /// let basic = style.to_string(Some(&config_fixed));
    ///
    /// // True color mode preserves RGB values
    /// let rgb = style.to_string(Some(&config_true));
    ...
    ///
    /// let mut output = String::new();
    /// output.push_str(&style.to_string(Some(&config)));
    ...
    pub fn to_string(&self, mode: Option<&AnsiConfig>) -> String {
        let mut output = String::default();
        Self::write_style(self, &mut output, mode).unwrap();
        output
    }
...
///
/// ```
/// use termionix_ansicodes::{StyledString, ColorMode, Style, Color, Intensity, AnsiConfig};
///
/// let config = AnsiConfig::enabled();
...
pub enum ColorMode {
    ...
    ///
    /// ```
    /// use termionix_ansicodes::{StyledString, ColorMode, AnsiConfig};
    ///
    ...
    ///
    /// ```
    /// use termionix_ansicodes::{StyledString, ColorMode, Style, Color, AnsiConfig};
    ///
    /// let config = AnsiConfig::enabled();
...
mod tests {
    ...
    fn test_color_fixed_boundary_values() {
        ...
        style_0.write_style(&mut output_0, Some(&config)).unwrap();
        style_255
            .write_style(&mut output_255, Some(&config))
            .unwrap();
    ...
    fn test_color_rgb_boundary_values() {
        ...
        style_min
            .write_style(&mut output_min, Some(&config))
            .unwrap();
        style_max
            .write_style(&mut output_max, Some(&config))
            .unwrap();
    ...
    fn test_style_len() {
        ...
        let style = Style::default();
        assert_eq!(style.len(Some(&config)), 0);
        ...
        // \x1b[ (2) + 1 (1) + m (1) = 4
        assert_eq!(style.len(Some(&config)), 4);
        ...
        // \x1b[ (2) + 31 (2) + m (1) = 5
        assert_eq!(style.len(Some(&config)), 5);
        ...
        assert_eq!(style.len(Some(&config)), 18);
    }

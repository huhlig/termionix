...
///
/// ```rust
/// use termionix_ansicodes::{Style, Color, ColorMode, Intensity};
/// use std::fmt::Write;
///
...
/// style.write_style(&ColorMode::Basic, &mut output).unwrap();
/// write!(&mut output, "Bold Red Text").unwrap();
/// Style::write_reset(&ColorMode::Basic, &mut output).unwrap();
/// ```
///
/// ## Complex Styling
///
/// ```rust
/// use termionix_ansicodes::{Underline,Style, Color, ColorMode, Intensity};
///
/// let style = Style {
...
impl Style {
    ...
    ///
    /// ```rust
    /// use termionix_ansicodes::{ColorMode, Style};
    /// use std::fmt::Write;
    ...
    /// let mut output = String::new();
    ///
    /// Style::write_reset(&mode, &mut output).unwrap();
    /// assert_eq!(output, "\x1b[0m"); // Check that the ANSI reset code is written
    ///
    /// ```
    pub fn write_reset<W: std::fmt::Write>(mode: &ColorMode, writer: &mut W) -> std::fmt::Result {
        if !mode.is_ansi() {
            return Ok(());
        }
        write!(writer, "\x1b[0m")
    ...
    ///
    /// ```rust
    /// use termionix_ansicodes::{Style, Color, ColorMode, Intensity, Underline, Blink};
    /// use std::fmt::Write;
    ///
    /// let style = Style {
    ///     intensity: Some(Intensity::Bold),
    ...
    ///
    /// let mut output = String::new();
    /// style.write_style(&ColorMode::TrueColor, &mut output).unwrap();
    ...
    /// ```
    pub fn write_style<W: std::fmt::Write>(
        &self,
        mode: &ColorMode,
        writer: &mut W,
    ) -> std::fmt::Result {
    ...
    /// Converts the style into its ANSI escape sequence string representation.
    ///
    /// This is a convenience method that creates a string containing the ANSI escape codes
    /// for this style, based on the specified color mode. It internally calls [`write_style`]
    /// and collects the output into a `String`.
    ///
    /// # Parameters
    ///
    /// - `mode`: A reference to a [`ColorMode`] that determines which ANSI escape sequences
    ///   should be generated based on terminal capabilities.
    ///
    /// # Returns
    ///
    /// A `String` containing the ANSI escape sequence for this style. If the color mode
    /// is [`ColorMode::None`] or if the style has no attributes set, returns an empty string.
    ///
    /// # Examples
    ///
    ...
    /// };
    ///
    /// let ansi_code = style.to_string(&ColorMode::Basic);
    /// assert_eq!(ansi_code, "\x1b[1;31m");
    /// ```
    ///
    /// ## Empty Style
    ///
    /// ```rust
    /// use termionix_ansicodes::{Style, ColorMode};
    ///
    /// let style = Style::default();
    /// let ansi_code = style.to_string(&ColorMode::Basic);
    /// assert_eq!(ansi_code, "");
    ...
    ///
    /// ```rust
    /// use termionix_ansicodes::{Style, Color, ColorMode};
    ///
    /// let style = Style {
    ///     foreground: Some(Color::RGB(255, 100, 50)),
    ///     ..Default::default()
    /// };
    ///
    /// // No output when colors are disabled
    /// assert_eq!(style.to_string(&ColorMode::None), "");
    ///
    /// // Basic colors convert RGB to nearest basic color
    /// let basic = style.to_string(&ColorMode::Basic);
    ///
    /// // True color mode preserves RGB values
    /// let rgb = style.to_string(&ColorMode::TrueColor);
    /// assert_eq!(rgb, "\x1b[38;2;255;100;50m");
    /// ```
    ///
    /// ## Combining with Text
    ///
    ...
    ///
    /// let mut output = String::new();
    /// output.push_str(&style.to_string(&ColorMode::Basic));
    /// output.push_str("Success!");
    /// output.push_str("\x1b[0m"); // Reset
    ///
    /// // Result: "\x1b[1;32mSuccess!\x1b[0m"
    /// ```
    ///
    /// # Notes
    ///
    /// - This method allocates a new `String` each time it's called. For performance-critical
    ///   code that writes to an existing buffer, consider using [`write_style`] directly.
    /// - The returned string does not include a reset sequence. Use [`write_reset`] or
    ///   append `"\x1b[0m"` manually to clear the style after your text.
    ///
    /// # Panics
    ///
    /// This method unwraps the result of [`write_style`], which writes to a `String`.
    /// Since writing to a `String` is infallible in practice, this should never panic.
    ///
    /// # See Also
    ///
    /// - [`write_style`] - For writing directly to a buffer without allocation
    /// - [`write_reset`] - For writing the reset sequence
    /// - [`ColorMode`] - For understanding color mode capabilities
    ///
    /// [`write_style`]: Style::write_style
    /// [`write_reset`]: Style::write_reset
    pub fn to_string(&self, mode: &ColorMode) -> String {
        let mut output = String::default();
        Self::write_style(self, mode, &mut output).unwrap();
        output
    }
...
///
/// ```
/// use termionix_ansicodes::{Intensity, Style, StyledString, ColorMode};
///
/// // Create bold text
...
///
/// ```rust
/// use termionix_ansicodes::{Underline, Style, StyledString, ColorMode};
///
...
/// let mode = ColorMode::Basic;
/// let mut output = String::new();
/// styled.write_str(&mode, &mut output).unwrap();
/// ```
///
/// # Default
///
...
/// - [`Style`] - Container for text styling including colors
...
///
/// ```
/// use termionix_ansicodes::{StyledString, ColorMode, Style, Color, Intensity};
///
...
/// let mut output = String::new();
/// styled.write_str(&ColorMode::TrueColor, &mut output).unwrap();
/// ```
///
/// Detecting terminal capabilities:
///
/// ```
/// use termionix_ansicodes::ColorMode;
///
...
///
/// ```
/// use termionix_ansicodes::{ColorMode, Color};
///
...
pub enum ColorMode {
    ...
    ///
    /// ```
    /// use termionix_ansicodes::{StyledString, ColorMode};
    ///
    /// let styled = StyledString::from_string("Plain text", None);
    /// let mut output = String::new();
    /// styled.write_str(&ColorMode::None, &mut output).unwrap();
    ...
    ///
    /// ```
    /// use termionix_ansicodes::{StyledString, ColorMode, Style, Color};
    ///
    ...
    ///
    /// let mut output = String::new();
    /// styled.write_str(&ColorMode::Basic, &mut output).unwrap();
    ...
    ///
    /// let mut output = String::new();
    /// styled.write_str(&ColorMode::FixedColor, &mut output).unwrap();
...
///
/// ```
/// use termionix_ansicodes::{Style, Font, ColorMode};
///
/// let style = Style {
...
///
/// let mut output = String::new();
/// fraktur_style.write_style(&ColorMode::Basic, &mut output).unwrap();
/// // Generates: "\x1b[20m"
/// ```
///
...
///
/// let mut output = String::new();
/// reset_style.write_style(&ColorMode::Basic, &mut output).unwrap();
/// // Generates: "\x1b[10m"
/// ```
///
/// # Default
...
/// # Notes
///
/// - Setting `font: None` in a [`Style`] means no font selection code is generated
...
/// # See Also
///
/// - [`Style`] - Container for text styling including font selection
/// - [`Style::font`] - The font field in a style
/// - [`Style::write_style`] - Renders font selection as ANSI codes
#[derive(Clone, Debug, Default, PartialEq, Eq, Hash)]
pub enum Font {
...
impl Font {
    ...
    /// # See Also
    ///
    /// - [`Style::write_style`] - Uses this method when rendering font codes
    /// - [`Style::font`] - The field that stores font selection in a style
    pub fn to_u8(&self) -> u8 {
        match self {
            Font::PrimaryFont => 10,
    ...
    /// - The method is used internally by [`Style::parse`] when processing SGR parameters
    ///
    /// # See Also
    ///
    /// - [`to_u8()`](Font::to_u8) - Converts a `Font` to its SGR code
    /// - [`Style::parse`] - Parses complete SGR sequences including font codes
...
///
/// ```rust
/// use termionix_ansicodes::{Style, Blink, ColorMode};
///
/// let style = Style {
...
/// };
///
/// let output = style.to_string(&ColorMode::Basic);
...
/// let mut output = String::new();
///
/// slow.write_style(&mode, &mut output).unwrap();
/// assert_eq!(output, "\x1b[5m");
///
/// output.clear();
/// rapid.write_style(&mode, &mut output).unwrap();
...
/// # See Also
///
/// - [`Style`] - Container for all text styling attributes including blink
...
///
/// ```
/// use termionix_ansicodes::{Style, Script, ColorMode};
///
...
///
/// let mut output = String::new();
/// superscript_style.write_style(&ColorMode::Basic, &mut output).unwrap();
/// // Generates: "\x1b[73m"
/// ```
///
/// Subscript usage:
...
///
/// let mut output = String::new();
/// subscript_style.write_style(&ColorMode::Basic, &mut output).unwrap();
/// // Generates: "\x1b[74m"
/// ```
///
...
///
/// let mut output = String::new();
/// normal_style.write_style(&ColorMode::Basic, &mut output).unwrap();
/// // Generates: "\x1b[75m"
/// ```
///
...
/// # See Also
///
/// - [`Style`] - Container for text styling including script positioning
...
impl Script {
    ...
    /// - [`Style::write_style`] - Uses this method when rendering script codes
    pub fn to_u8(&self) -> u8 {
        match self {
            Script::Superscript => 73,
            Script::Subscript => 74,
    ...
    /// - [`Style::parse`] - Parses complete SGR sequences including script codes
    /// - [`Intensity::from_u8`] - Similar conversion for intensity codes
...
///
/// ```
/// use termionix_ansicodes::{Style, Ideogram, ColorMode};
///
...
///
/// let mut output = String::new();
/// ideogram_style.write_style(&ColorMode::Basic, &mut output).unwrap();
...
///
/// ```
/// use termionix_ansicodes::{Ideogram, Style, ColorMode};
///
...
///
/// let mut output = String::new();
/// emphasis_style.write_style(&ColorMode::Basic, &mut output).unwrap();
/// // Generates: "\x1b[63m"
/// ```
///
/// Stress marking:
...
///
/// let mut output = String::new();
/// stress_style.write_style(&ColorMode::Basic, &mut output).unwrap();
/// // Generates: "\x1b[64m"
/// ```
///
...
/// - Setting `ideogram: None` in a [`Style`] means no ideogram code is generated
///
/// # Historical Context
///
...
/// # See Also
///
/// - [`Style`] - Container for text styling including ideogram decorations
...
impl Ideogram {
    ...
    /// - [`Style::write_style`] - Uses this method when rendering ideogram codes
    pub fn to_u8(&self) -> u8 {
        match self {
    ...
    /// - [`Style::parse`] - Parses complete SGR sequences including ideogram codes
    /// - [`Font::from_u8`] - Similar conversion for font codes
...
mod tests {
    ...
    #[test]
    fn test_write_reset_no_color_mode() {
    ...
    fn test_write_style_empty_style() {
        ...
        style.write_style(&mode, &mut output).unwrap();
        assert_eq!(output, "");
    }
    ...
    fn test_write_style_with_different_color_modes() {
        ...
        output.clear();
        style
            .write_style(&ColorMode::FixedColor, &mut output)
            .unwrap();
        assert_eq!(output, "\x1b[31m");
        ...
        output.clear();
        style
            .write_style(&ColorMode::TrueColor, &mut output)
            .unwrap();
        assert_eq!(output, "\x1b[31m");

        // Test with None mode
        output.clear();
        style.write_style(&ColorMode::None, &mut output).unwrap();
        assert_eq!(output, "");
    }
    ...
    fn test_color_fixed_boundary_values() {
        ...
        style_0.write_style(&mode, &mut output_0).unwrap();
        style_255.write_style(&mode, &mut output_255).unwrap();
    ...
    fn test_color_rgb_boundary_values() {
        ...
        style_min.write_style(&mode, &mut output_min).unwrap();
        style_max.write_style(&mode, &mut output_max).unwrap();

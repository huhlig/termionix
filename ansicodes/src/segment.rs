...
use crate::{CSICommand, ControlCode, SpannedString, Style};
        ...
        for segment in &self.0 {
            match segment {
                ...
                Segment::Control(_) => {
                    ...
                    if let Some(config) = config && config.control {
                ...
                Segment::CSI(_) => {
                    ...
                    if let Some(config) = config && config.csi {
    ...
    pub fn write_str<W: std::fmt::Write>(
        ...
        for segment in &self.0 {
            match segment {
                ...
                Segment::CSI(command) => {
                    // Write CSI sequences
                    command.write_csi(writer, config)?;
                }
                Segment::SGR(style) => {
                    // Write style codes based on color mode
                    style.write_style(writer, config)?;
                }
                Segment::OSC(data)
                | Segment::DCS(data)
                | Segment::SOS(data)
...
mod tests {
    ...
    fn test_styled_len_empty() {
        ...
        assert_eq!(seg.styled_len(Some(&config)), 0);
    }
    ...
    fn test_styled_len_plain_text() {
        ...
        let config = AnsiConfig::default();
        let len = seg.styled_len(Some(&config));
    ...
    fn test_styled_len_with_style() {
        ...
        assert_eq!(seg.styled_len(Some(&config)), 11); // More than just "Text"
    }
    ...
    fn test_write_str_empty() {
        ...
        seg.write_str(&mut output, Some(&config)).unwrap();
        assert_eq!(output, "");
    }

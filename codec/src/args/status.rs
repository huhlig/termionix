...
use crate::{CodecError, CodecResult, TelnetOption, args::naws::WindowSize, consts};
...
impl StatusCommand {
    ...
    pub fn from_byte(byte: u8) -> CodecResult<Self> {
        match byte {
            ...
            _ => Err(CodecError::SubnegotiationError(format!(
...
impl TelnetOptionStatus {
    ...
    fn encode<T: BufMut>(&self, dst: &mut T) -> CodecResult<()> {
        dst.put_u8(self.command.to_byte());

        for (option, (do_state, will_state)) in &self.options {
            // Encode DO/DONT state
            if *do_state {
                dst.put_u8(consts::DO); // DO
            } else {
                dst.put_u8(consts::DONT); // DONT
            }
            dst.put_u8(option.as_u8());

            // Encode WILL/WONT state
            if *will_state {
                dst.put_u8(consts::WILL); // WILL
            } else {
                dst.put_u8(consts::WONT); // WONT
            }
            dst.put_u8(option.as_u8());
        }
        Ok(())
    ...
    ///
    fn decode<T: Buf>(src: &mut T) -> CodecResult<TelnetOptionStatus> {
        if src.remaining() < 1 {
            return Err(CodecError::SubnegotiationError(
                "Status decode error: insufficient data for command".into(),
            ));
        }
        ...
        if matches!(command, StatusCommand::Send) && src.remaining() > 0 {
            return Err(CodecError::SubnegotiationError(
                "Status SEND should not contain option data".into(),
            ));
        }
        ...
        while src.remaining() >= 2 {
            ...
            let option = TelnetOption::try_from(option_code).map_err(|_| {
                CodecError::SubnegotiationError(format!("Unknown option code: {}", option_code))
            })?;
        ...
        if src.remaining() > 0 {
            return Err(CodecError::SubnegotiationError(
                "Status decode error: incomplete option pair".into(),
            ));
        }
...
mod tests {
    ...
    fn test_telnet_option_status_decode_send_with_data_should_fail() {
        ...
        buf.put_u8(TelnetOption::Echo.as_u8());
    ...
    fn test_telnet_option_status_decode_is_with_options() {
        ...
        buf.put_u8(TelnetOption::SuppressGoAhead.as_u8());
    ...
    fn test_telnet_option_status_encoded_len() {
        ...
        assert_eq!(status.encoded_len(), expected_len);

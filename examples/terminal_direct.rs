...
async fn handle_connection(
    ...
    framed.send("\r\n").await?;
    framed.send("╔══════════════════════════════════════╗\r\n").await?;
    framed.send("║   Terminal Codec Direct Demo        ║\r\n").await?;
    framed.send("╚══════════════════════════════════════╝\r\n").await?;
    framed.send("\r\n").await?;
    framed.send("This example uses the TerminalCodec directly.\r\n").await?;
    framed.send("Type anything and it will be echoed back.\r\n").await?;
            ...
            Ok(event) => {
                match event {
                    TerminalEvent::CharacterData { character, cursor } => {
                        char_count += 1;
                        println!("[Connection {}] Character: '{}' at {:?}", conn_id, character, cursor);
                    ...
                    TerminalEvent::LineCompleted { line, cursor } => {
                        ...
                        println!("[Connection {}] Line completed: '{}' at {:?}", conn_id, text, cursor);
                        
                        // Check for quit command
                        if text.trim().eq_ignore_ascii_case("quit") {
                            framed.send("\r\n\r\nGoodbye!\r\n").await?;
                            let stats_msg = format!("Stats: {} characters, {} lines\r\n", char_count, line_count);
                        ...
                        let stats_prompt = format!("(chars: {}, lines: {})\r\n> ", char_count, line_count);
                        framed.send(stats_prompt.as_str()).await?;
                    }
                    TerminalEvent::ResizeWindow { old, new } => {
                        println!("[Connection {}] Window resized from {}x{} to {}x{}",
